...
describe('stress: large payload resilience', () => {
  ...
  it('handles large event payload (100KB+) without crash', async () => {
    ...
    await withClient(hub.baseUrl, async (c) => {
      ...
      const evs = polled.events as { payload: { content?: string; type?: string } }[];
      const largeEvent = evs.find(
        (e) => e.payload?.type === 'large_data' && e.payload?.content?.length === 100_000,
      );
