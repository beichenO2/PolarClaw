...
import { gsdConfigSchema } from '../protocol/config.js';
...
export function loadConfigFromDisk(root: string): GsdConfig {
  ...
  return gsdConfigSchema.parse(raw);
}

export function watchConfig(root: string, onChange: (cfg: GsdConfig) => void): () => void {
  const p = configFilePath(root);
...
export function updateConfigOnDisk(root: string, input: HubUpdateConfigInput): HubUpdateConfigOutput {
  ...
  const parsed = gsdConfigSchema.parse(next);
