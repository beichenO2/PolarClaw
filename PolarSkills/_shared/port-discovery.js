...
export function getCircuit(serviceName) {
 ...
 */
export async function isHealthy(url, timeoutMs = 3000) {
    try {
...
export function getGatewayUrl(servicePrefix) {
    return `${SOTAGENT_BASE}/gw/${servicePrefix.toLowerCase()}`;
}
export async function getServiceUrl(serviceName, gatewayPrefix) {
    if (gatewayPrefix) {
 ...
 */
export async function resilientFetch(serviceName, url, opts = {}, timeoutMs = 10000) {
