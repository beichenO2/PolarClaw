...
describe('Step 1: ClawMem 清理验证', () => {
  it('KnowLever/ 中不应包含 clawmem 引用', () => {
    const result = execSync(
      ...
      { cwd: '/Users/mac/Polarisor', encoding: 'utf-8', timeout: 30000 }
    );
  ...
  it('PolarClaw/ 中不应包含 clawmem 引用', () => {
    const result = execSync(
      ...
      { cwd: '/Users/mac/Polarisor', encoding: 'utf-8' }
    );
    const files = result.trim().split('\n').filter(Boolean);
    expect(files.length).toBe(0);
  });
...
describe('Step 2: KnowLever 关键信息保护', () => {
  it('compressContext 保护机制应保留关键信息', () => {
    const result = execSync(
      ...
      { cwd: '/Users/mac/Polarisor/KnowLever', encoding: 'utf-8' }
    );
...
describe('Step 5: 时间衰减权重', () => {
  ...
  it('7 天前权重应 < 0.5', () => {
    const result = execSync(
      `cd /Users/mac/Polarisor/KnowLever && python3 -c "
import datetime
