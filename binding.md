...
3. **Append 而非 Replace**：入口特化 Prompt 通过 PolarPrivate 的 `append_system_prompt` 机制注入，叠加在 persona 和 worker.md 之上，不替换它们。层次：skills/SOUL.md（生态地图）→ worker.md（Agent 身份约束）→ persona（人格风格）→ entry prompt（交互风格，不限能力）。
