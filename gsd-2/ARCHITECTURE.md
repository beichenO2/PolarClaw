...
- **单次执行架构**：每个 CLI Agent 启动一次（`cursor agent --print`），在内部循环中持续运行（不断调用 Shell 工具），直到 context window 耗尽才自然死亡。死亡后 tmux session 结束，由备用池 Agent 接替。不会重启同一个 Agent
- **Agent 是职员不是螺丝**：每个 Agent 长期复用，积累上下文和模块经验
- **消息驱动 + 时钟兜底**：正常情况下消息驱动工作流转，CLK 时钟防止死锁
- **故障自愈**：任何角色死亡后，从备用池中继任（不算新建 Agent）
...
- **凡构建必部署**：gsd-2 代码有改动 → 必须重新部署（停旧集群 → 启新集群）
- **凡部署必最新**：部署前必须 `git pull` 拿到最新版本
- **默认必须重启**：有改动就重启集群，除非用户明确声明「不准重启」
- **交付即推送**：阶段验证 PASS 后必须 git push 到 GitHub 对应私有仓库
...
5. **凡构建必部署、凡部署必最新、默认必须重启**：
   - 拉取更新后，停掉旧集群（`stop-cluster.sh`）→ 用最新代码重新启动集群（`launch-cluster.sh`）
   - 除非用户明确声明「不准重启」，否则有改动就必须重启
   - 重启前确保 `git pull` 拿到最新版本
